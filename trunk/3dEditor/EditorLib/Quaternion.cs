...
namespace EditorLib
{
    ...
    public class Quaternion
    {
        ...
        public Quaternion(double w, Point3D p)
        {
            W = w;
            X = p.X;
            Y = p.Y;
        ...
        public Quaternion(Point3D axis, double thetaDegs)
        {
            Point3D ax = axis.Normalize();
            double thetaPul = EditorMath.Degrees2Rad(thetaDegs) / 2;
            W = Math.Cos(thetaPul);
        ...
        public double[] ToEulerDegs()
        {
            ...
            fi = EditorMath.Radians2Deg(fi);
            double theta = Math.Asin(2 * (W * Y - Z * X));
            theta = EditorMath.Radians2Deg(theta);
            double psi = Math.Atan2(2 * (W * Z + X * Y), 1 - 2 * (Y * Y + Z * Z));
            psi = EditorMath.Radians2Deg(psi);
        ...
        public static void TestQuaternion1()
        {
            ...
            Quaternion q = new Quaternion(new Point3D(0, 0, 1), 90);
            matrixOwn = q.Matrix();
            Quaternion qq = new Quaternion(new Point3D(0, 1, 0), 40);
            matrixOwn = qq.Matrix() * matrixOwn;
            Quaternion qqq = new Quaternion(new Point3D(1, 0, 0), 10);
